...
import (
	...
	omni "github.com/voxgig-sdk/api2news-endpoint-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.Api2newsEndpointError); is {
				out := map[string]any{
					"name":    "Api2newsEndpointError",
...
func omniCtx(arg any, client *sdk.Api2newsEndpointSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.Api2newsEndpointSDK); is {
			client = live
