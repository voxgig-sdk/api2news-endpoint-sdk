...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("API2NEWS_ENDPOINT_TEST_LIVE") == "TRUE" ||
		os.Getenv("API2NEWS_ENDPOINT_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("API2NEWS_ENDPOINT_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.Api2newsEndpointSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.Api2newsEndpointError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.Api2newsEndpointSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.Api2newsEndpointError{Msg: msg, Code: code}
}
