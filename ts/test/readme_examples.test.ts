...
//     (\`new Api2newsEndpointSDK(...)\` / \`Api2newsEndpointSDK.test(...)\` become
//     \`Api2newsEndpointSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'Api2newsEndpointSDK'
...
const TEST_SEED = {"entity":{"bbc":{"test01":{"id":"test01"}},"cnn":{"test01":{"id":"test01"}},"new":{"test01":{"id":"test01"}},"techcrunch":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new Api2newsEndpointSDK(...)\` and any
// \`Api2newsEndpointSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+Api2newsEndpointSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/Api2newsEndpointSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
