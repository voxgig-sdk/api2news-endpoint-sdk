...
module Api2newsEndpointUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = Api2newsEndpointResponse.new({})
    ...
    if fetch_err
      ...
      response = Api2newsEndpointResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = Api2newsEndpointResponse.new(fetched)
    else
