...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = Api2newsEndpointOmni.make_runner(TEST_JSON_FILE, Api2newsEndpointSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    Api2newsEndpointError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : Api2newsEndpointOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = Api2newsEndpointOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
