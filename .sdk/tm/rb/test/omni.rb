...
module Api2newsEndpointOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if Api2newsEndpointOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = Api2newsEndpointOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| Api2newsEndpointOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || Api2newsEndpointOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = Api2newsEndpointSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = Api2newsEndpointError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { Api2newsEndpointOmni.errify(err) }
