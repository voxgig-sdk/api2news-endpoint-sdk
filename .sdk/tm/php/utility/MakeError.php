...
class Api2newsEndpointMakeError
{
    public static function call(?Api2newsEndpointContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new Api2newsEndpointContext([], null);
        }
        $op = $ctx->op ?? new Api2newsEndpointOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new Api2newsEndpointResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof Api2newsEndpointError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new Api2newsEndpointError('', $msg, $ctx);
        ...
        if ($err instanceof Api2newsEndpointError) {
