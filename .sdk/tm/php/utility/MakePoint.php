...
// Api2newsEndpoint SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class Api2newsEndpointMakePoint
{
    public static function call(Api2newsEndpointContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof Api2newsEndpointError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = Api2newsEndpointHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = Api2newsEndpointHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
