...
from api2newsendpoint_sdk.api2newsendpoint_types import (
    Techcrunch,
    TechcrunchListMatch,
)
...
class TechcrunchEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TechcrunchEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Techcrunch]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Techcrunch().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
