...
class TestCnnDirect:
    ...
    def test_should_direct_list_cnn(self):
        setup = _cnn_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-cnn", "live" if setup["live"] else "unit")
        if _skip:
