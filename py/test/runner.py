# Api2newsEndpoint SDK test runner
...
class Api2newsEndpointTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            Api2newsEndpointTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = Api2newsEndpointTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = Api2newsEndpointTestRunner.getenv("API2NEWS_ENDPOINT_TEST_LIVE")
        override = Api2newsEndpointTestRunner.getenv("API2NEWS_ENDPOINT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = Api2newsEndpointTestRunner.getenv(key)
        ...
        explain = Api2newsEndpointTestRunner.getenv("API2NEWS_ENDPOINT_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if Api2newsEndpointTestRunner._test_control is not None:
            return Api2newsEndpointTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                Api2newsEndpointTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            Api2newsEndpointTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = Api2newsEndpointTestRunner.load_test_control()
...
def load_env_local():
    Api2newsEndpointTestRunner.load_env_local()


def env_override(m):
    return Api2newsEndpointTestRunner.env_override(m)


def entity_data(v):
    return Api2newsEndpointTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return Api2newsEndpointTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return Api2newsEndpointTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return Api2newsEndpointTestRunner.load_test_control()


def live_client_options():
    return Api2newsEndpointTestRunner.live_client_options()


def live_delay_ms():
    return Api2newsEndpointTestRunner.live_delay_ms()
