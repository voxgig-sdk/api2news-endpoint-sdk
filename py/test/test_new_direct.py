...
class TestNewDirect:
    ...
    def test_should_direct_list_new(self):
        setup = _new_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-new", "live" if setup["live"] else "unit")
        if _skip:
