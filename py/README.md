...
The Python SDK for the Api2newsEndpoint API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Bbc(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Api2newsEndpoint(None).load(
    {"id": "test01"}, None
)
