...
require("api2news-endpoint_types")
...
function Api2newsEndpointSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function Api2newsEndpointSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "Api2newsEndpointSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function Api2newsEndpointSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function Api2newsEndpointSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "Api2newsEndpointSDK: graphql: " .. msg
    res.graphql = errors
  end
