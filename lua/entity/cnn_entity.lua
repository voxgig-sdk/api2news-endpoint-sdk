...
local CnnEntity = {}
CnnEntity.__index = CnnEntity


function CnnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CnnEntity)
  self._name = "cnn"
  self._client = client
...
function CnnEntity:get_name()
  return self._name
end


function CnnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CnnEntity.new(self._client, opts)
end
...
function CnnEntity:mark_deleted()
  self._deleted = true
end


function CnnEntity:deleted()
  return true == self._deleted
end


function CnnEntity:data_set(args)
  if args ~= nil then
...
function CnnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CnnEntity:match_set(args)
  if args ~= nil then
...
function CnnEntity:stream(action, args, callopts)
...
function CnnEntity:_run_op(ctx, post_done)
  local utility = self._utility
