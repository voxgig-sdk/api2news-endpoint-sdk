...
function NewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NewEntity)
  self._name = "new"
  self._client = client
...
function NewEntity:get_name()
  return self._name
end


function NewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NewEntity.new(self._client, opts)
end
...
function NewEntity:mark_deleted()
  self._deleted = true
end


function NewEntity:deleted()
  return true == self._deleted
end


function NewEntity:data_set(args)
  if args ~= nil then
...
function NewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NewEntity:match_set(args)
  if args ~= nil then
...
function NewEntity:stream(action, args, callopts)
...
---@param reqmatch NewListMatch
---@param ctrl? table
---@return New[]
---@return string? err
function NewEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function NewEntity:_run_op(ctx, post_done)
  local utility = self._utility
