...
local TechcrunchEntity = {}
TechcrunchEntity.__index = TechcrunchEntity


function TechcrunchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TechcrunchEntity)
  self._name = "techcrunch"
  self._client = client
...
function TechcrunchEntity:get_name()
  return self._name
end


function TechcrunchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TechcrunchEntity.new(self._client, opts)
end
...
function TechcrunchEntity:mark_deleted()
  self._deleted = true
end


function TechcrunchEntity:deleted()
  return true == self._deleted
end


function TechcrunchEntity:data_set(args)
  if args ~= nil then
...
function TechcrunchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TechcrunchEntity:match_set(args)
  if args ~= nil then
...
function TechcrunchEntity:match_get()
...
function TechcrunchEntity:stream(action, args, callopts)
...
function TechcrunchEntity:_run_op(ctx, post_done)
  local utility = self._utility
