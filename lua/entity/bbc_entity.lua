...
local BbcEntity = {}
BbcEntity.__index = BbcEntity


function BbcEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BbcEntity)
  self._name = "bbc"
  self._client = client
...
function BbcEntity:get_name()
  return self._name
end


function BbcEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BbcEntity.new(self._client, opts)
end
...
function BbcEntity:mark_deleted()
  self._deleted = true
end


function BbcEntity:deleted()
  return true == self._deleted
end


function BbcEntity:data_set(args)
  if args ~= nil then
...
function BbcEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BbcEntity:match_set(args)
  if args ~= nil then
...
function BbcEntity:stream(action, args, callopts)
...
function BbcEntity:_run_op(ctx, post_done)
  local utility = self._utility
