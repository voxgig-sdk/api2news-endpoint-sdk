# Api2newsEndpoint Lua SDK



The Lua SDK for the Api2newsEndpoint API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Bbc()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── api2news-endpoint_sdk.lua    -- Main SDK module
...
The main module (`api2news-endpoint_sdk`) exports the SDK constructor
