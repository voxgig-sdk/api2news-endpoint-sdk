...
[MCP](https://modelcontextprotocol.io) server exposing the Api2newsEndpoint SDK as
two agent tools — `api2news-endpoint_list` and `api2news-endpoint_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/api2news-endpoint-mcp)
make build

# 2. Provide credentials via the environment
export API2NEWS_ENDPOINT_APIKEY=sk_live_xxx
...
```jsonc
// api2news-endpoint_list: first page of records
{ "entity": "bbc" }
{ "entity": "bbc", "query": { } }

// api2news-endpoint_load: one record by id
{ "entity": "bbc", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/api2news-endpoint-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user api2news-endpoint \
     -- "$PWD"/dist/*/api2news-endpoint-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `api2news-endpoint_list` and `api2news-endpoint_load` tools now appear
   in new sessions. Ask the agent to *"list bbc using api2news-endpoint"*
   and it calls `api2news-endpoint_list` with `{"entity":"bbc"}`.
...
```sh
export API2NEWS_ENDPOINT_APIKEY=sk_live_xxx            # API key
export API2NEWS_ENDPOINT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `api2news-endpoint_list` tool
...
### Call the `api2news-endpoint_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `api2news-endpoint_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `api2news-endpoint_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `API2NEWS_ENDPOINT_APIKEY` | API key sent with every request. |
| `API2NEWS_ENDPOINT_BASE` | Optional override of the API base URL. |
