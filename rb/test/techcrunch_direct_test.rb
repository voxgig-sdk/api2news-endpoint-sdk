...
class TechcrunchDirectTest < Minitest::Test
  def test_direct_list_techcrunch
    setup = techcrunch_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-techcrunch", setup[:live] ? "live" : "unit")
    if _should_skip
...
def techcrunch_direct_setup(mockres)
