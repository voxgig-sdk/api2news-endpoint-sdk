# Api2newsEndpoint Ruby SDK



The Ruby SDK for the Api2newsEndpoint API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Bbc` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = Api2newsEndpointSDK.test(testopts, sdkopts)
```
...
| `Bbc` | `(data) -> BbcEntity` | Create a Bbc entity instance. |
| `Cnn` | `(data) -> CnnEntity` | Create a Cnn entity instance. |
| `New` | `(data) -> NewEntity` | Create a New entity instance. |
| `Techcrunch` | `(data) -> TechcrunchEntity` | Create a Techcrunch entity instance. |
...
raise a `Api2newsEndpointError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Api2newsEndpoint_sdk.rb       -- Main SDK module
...
The main module (`Api2newsEndpoint_sdk`) exports the SDK class
...
# bbc.data_get now returns the bbc data from the last list
