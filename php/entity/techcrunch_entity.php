...
// Api2newsEndpoint SDK Techcrunch entity
...
class TechcrunchEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TechcrunchEntity($this->_client, $opts);
    }

    /**
     * @param Techcrunch|array $args Techcrunch data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = Api2newsEndpointHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Techcrunch|array The current Techcrunch data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Techcrunch fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = Api2newsEndpointHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Techcrunch fields).
     */
     ...
     *
     * @param TechcrunchListMatch|array|null $reqmatch Match filter (any subset
     *   of Techcrunch fields) as an assoc-array; TechcrunchListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Techcrunch[]|array A list of Techcrunch items as assoc-arrays at
     *   the SDK boundary; throws Api2newsEndpointError on failure (item-5 convention).
     */
