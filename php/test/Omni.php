...
//    only - a typed Api2newsEndpointContext there would make every ctx
...
final class Api2newsEndpointOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new Api2newsEndpointContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new Api2newsEndpointSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new Api2newsEndpointResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new Api2newsEndpointError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new Api2newsEndpointResponse($clean['response']);
    ...
    public static function err_from_map($m): ?Api2newsEndpointError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new Api2newsEndpointError(is_string($code) ? $code : '', $msg);
    }
