...
class TechcrunchEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Techcrunch(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Techcrunch(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set API2NEWS_ENDPOINT_TEST_TECHCRUNCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $techcrunch_ref01_list_result = $techcrunch_ref01_ent->list($techcrunch_ref01_match, null);
...
function techcrunch_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("API2NEWS_ENDPOINT_TEST_TECHCRUNCH_ENTID");
