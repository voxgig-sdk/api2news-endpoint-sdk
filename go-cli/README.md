# api2news-endpoint-cli

boru-driven command-line client **and** interactive REPL for the Api2newsEndpoint
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/api2news-endpoint-cli)
make build

# 2. See usage (words, entities, env vars)
./api2news-endpoint-cli --help

# 3. Provide credentials once, via the environment
export API2NEWS_ENDPOINT_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./api2news-endpoint-cli list bbc
./api2news-endpoint-cli list cnn

# 5. Override the API base URL for a single call
API2NEWS_ENDPOINT_BASE=https://api.example.com ./api2news-endpoint-cli list bbc

# 6. No arguments -> interactive REPL
./api2news-endpoint-cli
api2news-endpoint> list bbc
api2news-endpoint> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/api2news-endpoint-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/api2news-endpoint-cli list bbc
   ```
...
Run with no arguments to open a REPL (prompt `api2news-endpoint>`). Each line is
evaluated as its own boru expression:

```text
$ ./api2news-endpoint-cli
api2news-endpoint> list bbc
api2news-endpoint> /help
api2news-endpoint> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `bbc`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/api2news-endpoint-cli`. |
...
(`./api2news-endpoint-cli <expr>`) and interactively (the REPL), and expressions compose the
